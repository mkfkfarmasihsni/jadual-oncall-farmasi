...
// --- Konfigurasi Firebase yang Selamat ---
...
const App = () => {
  ...
  const [appSettings, setAppSettings] = useState({ title: 'E-OnCall Farmasi', subtitle: 'Hospital Sultanah Nora Ismail' });
  ...
  const saveStaff = async (e) => {
    ...
    try {
      const data = { 
        name: String(newStaff.name).toUpperCase(), 
        phone: String(newStaff.phone || ''), 
        unit: String(newStaff.unit || ''), 
      ...
      setNewStaff({ name: '', phone: '', unit: '', id: null }); 
      setShowStaffModal(false); 
      showStatus("Data staf disimpan!", "success");
  ...
  const deleteHoliday = async (id) => {
    if (!user) return;
    try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'holidays', id)); showStatus("Cuti dipadam", "info"); } catch (err) { showStatus("Gagal memadam", "error"); }
  };
    ...
    return (
      <div className="min-h-screen bg-white p-8 font-sans text-black text-left">
        ...
        <div className="mb-6 p-4 border border-black bg-slate-50">
          <h3 className="text-xs font-black uppercase mb-3 border-b border-black pb-1">Rujukan Waktu Bertugas</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-y-2 gap-x-6 text-black text-left">
            {filteredShiftDefs.map(def => (<div key={def.id} className="flex gap-2 text-[9px]"><span className="font-black whitespace-nowrap min-w-[100px]">{String(def.label)}:</span><span className="whitespace-pre-line leading-tight">{String(def.time || 'Waktu tidak set')}</span></div>))}
          </div>
        </div>
        <div className="border border-black text-black">
          ...
          <div className="grid grid-cols-7 bg-black gap-px">
            {calendarDays.map((dateStr, idx) => {
              ...
              const d = new Date(dateStr), h = holidays[dateStr], s = shifts[dateStr] || {};
              return (
                <div key={dateStr} className={`p-2 min-h-[120px] flex flex-col ${h ? 'bg-yellow-50' : 'bg-white'} text-left text-black`}>
                  ...
                  <div className="space-y-1">
                    {filteredShiftDefs.map(type => (s[type.id] || []).map(st => (<p key={st.staffId} className="text-[7px] font-medium text-black truncate leading-tight">• {String(st.staffName)} ({type.label})</p>)))}
                  </div>
                </div>
              );
            })}
        ...
        <div className="mt-20 grid grid-cols-2 gap-20 text-center text-black">
          <div className="space-y-12"><p className="text-xs font-bold uppercase underline leading-tight">Disediakan Oleh:</p><div><div className="border-b border-black w-64 mx-auto mb-1 text-black"></div><p className="text-[10px] font-black uppercase">(TANDATANGAN & COP RASMI)</p></div></div>
          <div className="space-y-12"><p className="text-xs font-bold uppercase underline leading-tight">Diluluskan Oleh:</p><div><div className="border-b border-black w-64 mx-auto mb-1 text-black"></div><p className="text-[10px] font-black uppercase">(TANDATANGAN & COP RASMI)</p></div></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 no-print flex flex-col">
      ...
      {statusMsg && (
        <div className={`fixed top-4 right-4 z-[500] p-4 rounded-xl shadow-xl flex items-center gap-2 animate-in slide-in-from-top-4 ${statusMsg.type === 'success' ? 'bg-emerald-600' : 'bg-slate-800'} text-white`}>
        ...
        {activeTab === 'calendar' ? (
             ...
             <div className="flex flex-col gap-4 text-slate-800 text-left">
              ...
              {activeShiftInfo && (
                <div className={`p-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] flex items-center gap-3 md:gap-4 border animate-in slide-in-from-top-2 ${getShiftColor(activeShiftInfo.color, 'light')} text-slate-800`}>
                  ...
                  <div className="text-left overflow-hidden"><h3 className="font-black text-xs md:text-sm uppercase leading-none mb-1 truncate font-black">{String(activeShiftInfo.label)}</h3><p className="text-[10px] md:text-xs font-bold opacity-80 uppercase tracking-widest whitespace-pre-line truncate">{String(activeShiftInfo.time)}</p></div>
                      ...
                      <div key={dateStr} onClick={() => setSelectedDate(dateStr)} className={`min-h-[80px] md:min-h-[220px] p-1 md:p-2 flex flex-col group cursor-pointer hover:bg-blue-50/30 transition-colors ${h ? 'bg-yellow-50' : 'bg-white'} ${today ? 'ring-2 ring-inset ring-blue-500 z-10' : ''}`}>
                        ...
                        <div className="space-y-0.5 md:space-y-1.5 overflow-hidden text-left text-slate-800">
                          {shiftDefinitions.map(type => {
                            ...
                            return (
                              <div key={type.id} onClick={(e) => { e.stopPropagation(); setActiveShiftType(type.id); }} className={`px-1 md:px-2 py-0.5 md:py-1.5 rounded-md md:rounded-xl text-[7px] md:text-[10px] font-black truncate border shadow-sm flex items-center gap-1 ${getShiftColor(type.color, 'light')} border-current/10 hover:brightness-95 transition-all text-slate-800`} title={`${String(type.label)}: ${String(type.time)}`}><div className={`w-1 md:w-1.5 h-1 md:h-1.5 rounded-full ${getShiftColor(type.color, 'bg')} shrink-0`}></div><span className="truncate uppercase">{staffList.length === 1 ? String(staffList[0].staffName).split(' ')[0] : `${staffList.length} STAF`}</span></div>
                            );
                          })}
                        </div>
                      </div>
          ...
          <div className="space-y-4 md:space-y-6 animate-in fade-in text-slate-800">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm gap-4 text-slate-800 text-left">
              <div className="text-left text-slate-800 uppercase leading-none"><h2 className="text-xl md:text-2xl font-black leading-tight uppercase text-left">Pergerakan Staf</h2><p className="text-slate-500 font-medium text-[10px] tracking-widest mt-1 uppercase text-left uppercase">Hospital Sultanah Nora Ismail</p></div>
              <button onClick={() => setShowMovementModal(true)} className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all uppercase text-xs tracking-widest leading-none"><span>Kemaskini</span></button>
            </div>
            <div className="bg-white p-3 md:p-4 rounded-xl border border-slate-200 shadow-sm flex flex-wrap gap-2 md:gap-4 items-center overflow-x-auto no-scrollbar text-left text-slate-800">
              <div className="flex items-center gap-1.5 mr-2 shrink-0"><Filter size={14} className="text-slate-400" /><span className="text-[8px] md:text-[10px] font-black uppercase text-slate-400 tracking-widest">Penapis:</span></div>
              <button onClick={() => setMovementFilter('all')} className={`px-3 py-1.5 rounded-full text-[9px] md:text-[11px] font-black uppercase transition-all ${movementFilter === 'all' ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>Semua</button>
              {MOVEMENT_TYPES.map(t => (<button key={t.id} onClick={() => setMovementFilter(prev => prev === t.id ? 'all' : t.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border-2 transition-all shrink-0 ${movementFilter === t.id ? `border-slate-800 ${t.color} text-white shadow-md` : `border-transparent bg-white ${movementFilter === 'all' ? 'opacity-100' : 'opacity-40'} hover:border-slate-200 text-slate-800`}`}><div className={`w-2 h-2 rounded-full ${movementFilter === t.id ? 'bg-white' : t.color}`}></div><span className={`text-[9px] md:text-[11px] font-bold uppercase ${movementFilter === t.id ? 'text-white' : 'text-slate-600'}`}>{t.label}</span></button>))}
            </div>
              ...
              <div className="grid grid-cols-7 gap-px bg-slate-100 text-slate-800">
                {calendarDays.map((dateStr, idx) => {
                  ...
                  const dayMovements = movements.filter(m => {
                    ...
                    const dateMatch = startOfThisDay >= start && startOfThisDay <= end;
                    const filterMatch = movementFilter === 'all' || m.type === movementFilter;
                    return dateMatch && filterMatch;
                  });
                  return (
                    <div key={dateStr} onClick={() => setViewingDayMovements(dateStr)} className="min-h-[100px] md:min-h-[160px] p-1 md:p-2 bg-white flex flex-col group transition-colors hover:bg-slate-50/50 text-slate-800 cursor-pointer overflow-hidden text-left"><span className="text-[10px] md:text-sm font-black text-slate-700 mb-1 md:mb-2">{parseInt(dateStr.split('-')[2])}</span><div className="space-y-0.5 md:space-y-1.5 overflow-hidden text-slate-800 text-left">{dayMovements.map(m => { const typeCfg = MOVEMENT_TYPES.find(t => t.id === m.type) || MOVEMENT_TYPES[0]; return (<div key={m.id} title={`${String(m.type)}: ${String(m.staffName)} ${m.type === 'Off' && m.timeInfo ? `(Waktu: ${m.timeInfo})` : ''}`} className={`p-1 md:p-2 rounded-md md:rounded-xl text-[7px] md:text-[9px] font-black text-white ${typeCfg.color} flex items-center justify-between shadow-sm transition-all text-left uppercase leading-none animate-in zoom-in-95`}><span className="truncate uppercase leading-tight">{String(m.staffName).split(' ')[0]}</span><button onClick={(e) => { e.stopPropagation(); deleteMovement(m.id); }} className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-black/10 rounded transition-all text-white shrink-0 ml-1"><X size={8} /></button></div>); })}</div></div>
                  );
                })}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl md:rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in fade-in text-slate-800 w-full">
            <div className="p-4 md:p-8 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50 text-slate-800">
              <div className="text-left uppercase font-black tracking-tight text-slate-800"><h2 className="text-xl md:text-2xl uppercase">Direktori</h2></div>
              <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                ...
                <button onClick={() => { setNewStaff({ name: '', phone: '', unit: '', id: null }); setShowStaffModal(true); }} className="bg-blue-600 text-white px-6 py-3 rounded-xl flex items-center justify-center gap-2 font-bold shadow-lg active:scale-95 transition-all uppercase text-xs tracking-widest leading-none"><UserPlus size={18} /><span>Tambah Staf</span></button>
              </div>
            </div>
            <div className="overflow-x-auto"><table className="w-full text-left text-slate-800">
              <tbody className="divide-y divide-slate-50 text-slate-800">
                {filteredStaffDirectory.map((person) => (
                  <tr key={person.id} className="hover:bg-slate-50/80 transition-colors uppercase">
                    <td className="px-4 md:px-6 py-4 text-left text-slate-800"><div className="flex items-center gap-3 text-left uppercase"><div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-black text-[10px] uppercase shrink-0">{person.name ? String(person.name).substring(0, 2) : '??'}</div><div><span className="font-bold text-xs md:text-sm block leading-tight uppercase truncate">{String(person.name)}</span><span className="text-[9px] font-black uppercase text-blue-500 tracking-wider block uppercase">{String(person.unit || 'Tiada Unit')}</span></div></div></td>
          ...
          <div className="space-y-6 animate-in fade-in text-slate-800 max-w-4xl mx-auto">
             <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 md:p-8 text-slate-800 text-left">
              <h2 className="text-xl md:text-2xl font-black mb-8 uppercase text-left leading-tight uppercase">Maklumat Jadual</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-slate-800 text-left uppercase">
                <div className="space-y-1 text-left"><label className="text-[10px] font-black uppercase text-slate-400 ml-1 block text-left uppercase leading-none">Tajuk Utama</label><input type="text" value={appSettings.title} onChange={(e) => setAppSettings({...appSettings, title: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none text-slate-800 shadow-inner" /></div>
                <div className="space-y-1 text-left"><label className="text-[10px] font-black uppercase text-slate-400 ml-1 block text-left uppercase leading-none">Sub-Tajuk</label><input type="text" value={appSettings.subtitle} onChange={(e) => setAppSettings({...appSettings, subtitle: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none text-slate-800 shadow-inner" /></div>
                <div className="md:col-span-2 flex justify-end mt-2"><button onClick={() => updateAppSettings(appSettings.title, appSettings.subtitle)} className="px-10 py-3.5 bg-slate-900 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all text-xs uppercase tracking-widest leading-none uppercase">Simpan Maklumat</button></div>
              </div>
            </div>
            <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 md:p-8 text-slate-800 text-left uppercase">
              <h2 className="text-xl md:text-2xl font-black mb-6 uppercase underline text-left uppercase">Cuti Umum</h2>
              <form onSubmit={addHoliday} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 text-slate-800">
                <input type="date" value={newHoliday.date} onChange={(e) => setNewHoliday({...newHoliday, date: e.target.value})} className="px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-700 outline-none shadow-inner uppercase" required />
                <input type="text" value={newHoliday.name} onChange={(e) => setNewHoliday({...newHoliday, name: e.target.value})} placeholder="Nama Cuti Umum..." className="px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-700 outline-none shadow-inner uppercase" required />
                <button type="submit" className="bg-blue-600 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all text-xs uppercase tracking-widest leading-none uppercase">Tambah</button>
              </form>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-left uppercase">
                {Object.values(holidays).sort((a,b) => String(a.date).localeCompare(String(b.date))).map(h => (<div key={h.id} className="flex items-center justify-between p-4 bg-yellow-50 border border-yellow-100 rounded-xl shadow-sm text-left uppercase font-black"><div><p className="text-xs font-black text-yellow-700 leading-tight uppercase font-black">{new Date(h.date).toLocaleDateString('ms-MY')}</p><p className="text-[10px] font-bold text-yellow-600 uppercase mt-0.5 leading-tight uppercase">{String(h.name)}</p></div><button onClick={() => deleteHoliday(h.id)} className="text-yellow-200 hover:text-red-500 transition-colors p-2 shrink-0"><Trash2 size={18} /></button></div>))}
              </div>
            </div>
            <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 md:p-8 text-slate-800 text-left uppercase">
              <div className="flex justify-between items-center mb-6"><div><h2 className="text-xl md:text-2xl font-black uppercase text-left leading-tight uppercase">Kategori Syif</h2></div><button onClick={() => { setEditingShiftDef({ label: '', time: '', color: 'blue' }); setShowShiftDefModal(true); }} className="bg-blue-600 text-white px-6 py-3 rounded-xl flex items-center gap-2 font-bold shadow-lg active:scale-95 text-xs uppercase tracking-widest leading-none uppercase"><Plus size={18} /><span>Tambah</span></button></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-slate-800 text-left uppercase">
                {shiftDefinitions.map(def => (
                  <div key={def.id} className="p-6 rounded-[2rem] border border-slate-100 bg-slate-50 group transition-all hover:bg-white hover:shadow-xl text-left text-slate-800 uppercase">
                    ...
                    <h3 className="font-black text-slate-800 text-xs md:text-sm uppercase leading-tight font-black uppercase">{String(def.label)}</h3><p className="text-[10px] font-bold text-slate-400 mt-1 uppercase whitespace-pre-line leading-tight text-left uppercase">{String(def.time || "Masa belum diset")}</p>
                    <div className="flex gap-2 mt-4 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all text-left uppercase"><button onClick={() => { setEditingShiftDef(def); setShowShiftDefModal(true); }} className="p-2 bg-white rounded-xl text-slate-600 hover:text-blue-600 border border-slate-100 shadow-sm"><Edit2 size={16} /></button><button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shiftDefinitions', def.id))} className="p-2 bg-white rounded-xl text-slate-600 hover:text-red-600 border border-slate-100 shadow-sm"><Trash2 size={16} /></button></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {viewingDayMovements && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden text-slate-800 flex flex-col text-left">
            <div className="p-8 border-b bg-slate-50/80 flex justify-between items-center text-slate-800 shrink-0 text-left uppercase">
              <div className="text-left leading-tight text-slate-800 uppercase font-black uppercase"><h3 className="text-xl font-black text-slate-800 leading-none text-left uppercase">Detail Harian</h3><p className="text-[10px] font-bold text-slate-500 mt-2 uppercase tracking-widest uppercase text-left leading-none">📅 {new Date(viewingDayMovements).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto no-scrollbar text-slate-800 flex-1 text-left uppercase">
              <div className="space-y-3 text-slate-800 text-left uppercase">
                {movements.filter(m => { const s = new Date(m.dateStart); s.setHours(0,0,0,0); const e = new Date(m.dateEnd); e.setHours(23,59,59,999); const curr = new Date(viewingDayMovements); return curr >= s && curr <= e; }).map(m => { const typeCfg = MOVEMENT_TYPES.find(t => t.id === m.type) || MOVEMENT_TYPES[0]; return (<div key={m.id} className={`p-4 rounded-2xl border border-transparent shadow-sm flex items-center justify-between ${typeCfg.light} animate-in zoom-in-95 uppercase`}><div className="flex flex-col text-left overflow-hidden uppercase"><span className="font-black uppercase text-xs truncate uppercase font-black">{String(m.staffName)}</span><div className="flex items-center gap-1.5 mt-1 opacity-75">{typeCfg.icon}<span className="font-bold text-[10px] uppercase tracking-wider">{m.type} {m.type === 'Off' && m.timeInfo ? `(${m.timeInfo})` : ''}</span></div></div><div className="text-[9px] font-bold uppercase opacity-50 text-right shrink-0 ml-4 uppercase leading-none">Aktif</div></div>); })}
                {movements.filter(m => { const s = new Date(m.dateStart); s.setHours(0,0,0,0); const e = new Date(m.dateEnd); e.setHours(23,59,59,999); const curr = new Date(viewingDayMovements); return curr >= s && curr <= e; }).length === 0 && <div className="py-12 text-center text-slate-400 italic uppercase leading-none">Tiada rekod harian.</div>}
              </div>
            </div>
            <div className="p-6 bg-slate-50 border-t shrink-0 uppercase"><button onClick={() => setViewingDayMovements(null)} className="w-full py-4 bg-slate-900 text-white font-black rounded-xl uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all">Tutup</button></div>
          </div>
        </div>
      )}
      ...
      {showMovementModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden text-slate-800 max-h-[95vh] flex flex-col text-left uppercase">
            <div className="p-6 border-b bg-slate-50/80 flex justify-between items-center text-slate-800 shrink-0 text-left uppercase font-black uppercase">
              <div className="text-left leading-tight text-slate-800 uppercase font-black uppercase"><h3 className="text-lg font-black text-slate-800 leading-none text-left uppercase">Kemaskini</h3><p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider text-left tracking-widest leading-none uppercase">Hospital Sultanah Nora Ismail</p></div>
              <button onClick={() => { setShowMovementModal(false); setMovementModalSearch(''); }} className="p-2 hover:bg-white rounded-full border border-slate-100 bg-white shadow-sm shrink-0"><X size={20} /></button>
            </div>
            <form onSubmit={saveMovement} className="p-6 space-y-5 overflow-y-auto no-scrollbar flex-1 text-slate-800 text-left uppercase">
              <div className="space-y-2 text-slate-800 text-left uppercase">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase text-left leading-none uppercase">Staf</label>
                {!newMovement.staffId ? (
                  <div className="relative text-slate-800 text-left uppercase font-black">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 uppercase" size={16} />
                    <input type="text" value={movementModalSearch} onChange={(e) => setMovementModalSearch(e.target.value)} placeholder="Taip nama..." className="w-full pl-10 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-sm text-slate-700 shadow-inner outline-none focus:ring-2 focus:ring-blue-500 uppercase font-black" />
                    {movementModalSearch && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-2xl z-[140] overflow-hidden max-h-48 overflow-y-auto text-left uppercase">
                        {filteredMovementStaff.map(s => (
                          <div key={s.id} onClick={() => { setNewMovement({...newMovement, staffId: s.id, staffName: String(s.name)}); setMovementModalSearch(''); }} className="p-3.5 hover:bg-blue-50 cursor-pointer border-b border-slate-50 transition-colors flex items-center gap-3 text-left uppercase text-slate-800 uppercase font-black leading-none"><div className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-[10px] uppercase shrink-0 uppercase font-black">{String(s.name).substring(0, 2)}</div><span className="font-bold text-xs text-slate-700 uppercase leading-none truncate uppercase font-black">{String(s.name)}</span></div>
                        ))}
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="p-3.5 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between text-slate-800 animate-in zoom-in-95 shadow-sm text-left uppercase font-black">
                    <div className="flex items-center gap-3 overflow-hidden text-left uppercase font-black"><div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-lg font-black text-xs shrink-0 font-black"><UserCheck size={16} /></div><span className="font-black text-blue-700 uppercase text-xs truncate leading-none text-left uppercase font-black leading-none">{String(newMovement.staffName)}</span></div>
              ...
              <div className="space-y-1.5 text-left text-slate-800 uppercase font-black uppercase">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase leading-none uppercase">Jenis</label>
                <div className="grid grid-cols-2 gap-2 text-slate-800 text-left uppercase">
                  {MOVEMENT_TYPES.map(type => (<button key={type.id} type="button" onClick={() => setNewMovement({...newMovement, type: type.id})} className={`p-2.5 rounded-xl text-[9px] font-black border-2 transition-all flex items-center gap-2 ${newMovement.type === type.id ? 'border-blue-600 bg-blue-50 text-blue-700 shadow-sm' : 'border-slate-100 bg-white text-slate-400 uppercase font-black'}`}><div className={`w-2 h-2 rounded-full shrink-0 ${type.color}`}></div>{type.label}</button>))}
                </div>
              </div>
              {newMovement.type === 'Off' && (
                <div className="space-y-1.5 animate-in slide-in-from-top-2 text-slate-800 text-left uppercase font-black"><label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase text-left leading-none uppercase leading-none">Masa Off (eg: 8am-12pm)</label><div className="relative text-left uppercase"><Clock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input type="text" value={newMovement.timeInfo} onChange={(e) => setNewMovement({...newMovement, timeInfo: e.target.value})} placeholder="Masukkan waktu..." className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs text-slate-700 shadow-inner outline-none uppercase font-black leading-none" /></div></div>
              )}
              <div className="grid grid-cols-2 gap-3 text-slate-800 text-left uppercase font-black">
                <div className="space-y-1 text-slate-800 text-left uppercase font-black"><label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase text-left leading-none uppercase leading-none">Mula</label><input type="date" value={newMovement.dateStart} onChange={(e) => setNewMovement({...newMovement, dateStart: e.target.value})} className="w-full px-3 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs text-slate-700 outline-none shadow-inner uppercase font-black leading-none" required /></div>
                <div className="space-y-1 text-slate-800 text-left uppercase font-black"><label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase text-left leading-none uppercase leading-none">Tamat</label><input type="date" value={newMovement.dateEnd} onChange={(e) => setNewMovement({...newMovement, dateEnd: e.target.value})} className="w-full px-3 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs text-slate-700 outline-none shadow-inner uppercase font-black leading-none" required /></div>
              </div>
              <button type="submit" className="w-full py-4 bg-slate-900 text-white font-black rounded-xl shadow-xl active:scale-95 transition-all disabled:opacity-50 mt-2 text-xs uppercase tracking-widest leading-none shadow-blue-900/10 uppercase font-black leading-none" disabled={!newMovement.staffId}>Simpan Pergerakan</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showStaffModal && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden p-6 animate-in slide-in-from-bottom-8 text-left uppercase">
            <div className="flex justify-between items-center mb-5 text-slate-800 font-black uppercase text-lg text-left leading-none uppercase"><h3 className="text-left uppercase">Maklumat Staf</h3><button onClick={() => setShowStaffModal(false)} className="p-1"><X size={20} /></button></div>
            <form onSubmit={saveStaff} className="space-y-4 text-slate-800 text-left uppercase">
              <input autoFocus type="text" value={newStaff.name} onChange={(e) => setNewStaff({...newStaff, name: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold focus:ring-2 focus:ring-blue-500 outline-none shadow-inner text-sm uppercase" placeholder="Nama Penuh..." required />
              <input type="tel" value={newStaff.phone} onChange={(e) => setNewStaff({...newStaff, phone: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold focus:ring-2 focus:ring-blue-500 outline-none shadow-inner text-sm uppercase" placeholder="WhatsApp..." />
              <input type="text" value={newStaff.unit} onChange={(e) => setNewStaff({...newStaff, unit: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold focus:ring-2 focus:ring-blue-500 outline-none shadow-inner text-sm uppercase" placeholder="Unit..." />
      ...
      {showShiftDefModal && editingShiftDef && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden p-6 animate-in zoom-in-95 text-slate-800 text-left uppercase flex flex-col">
            <div className="flex justify-between items-center mb-5 text-slate-800 font-black uppercase text-lg text-left leading-none uppercase"><h3 className="text-left uppercase">Tetapan Syif</h3><button onClick={() => setShowShiftDefModal(false)} className="p-1"><X size={20} /></button></div>
            <form onSubmit={saveShiftDef} className="space-y-4 text-slate-800 text-left uppercase">
              <input type="text" value={editingShiftDef.label} onChange={(e) => setEditingShiftDef({...editingShiftDef, label: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none text-sm text-slate-700 shadow-inner uppercase" placeholder="Nama Syif..." required />
              <textarea rows={3} value={editingShiftDef.time} onChange={(e) => setEditingShiftDef({...editingShiftDef, time: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none resize-none text-sm text-slate-700 shadow-inner uppercase" placeholder="Waktu Bertugas..." />
              <div className="flex flex-wrap gap-2 pt-1 text-slate-800 text-left uppercase">{COLOR_OPTIONS.map(c => (<button key={c.id} type="button" onClick={() => setEditingShiftDef({...editingShiftDef, color: c.id})} className={`w-8 h-8 rounded-lg border-4 ${c.bg} ${editingShiftDef.color === c.id ? 'border-slate-800 scale-110 shadow-md' : 'border-transparent'}`} />))}</div>
              <button type="submit" className="w-full py-4 bg-slate-900 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all text-xs uppercase tracking-widest mt-2 leading-none uppercase">Simpan Syif</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showPrintOptions && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden text-slate-800 flex flex-col uppercase font-black leading-none">
             <div className="p-6 border-b flex justify-between items-center bg-slate-50/80 text-slate-800 text-left shrink-0 font-black uppercase">
              <div className="text-left uppercase font-black tracking-tight leading-tight uppercase font-black"><h3 className="text-lg leading-none uppercase font-black">Cetak Laporan</h3></div>
              <button onClick={() => setShowPrintOptions(false)} className="p-2 bg-white rounded-full border shadow-sm shrink-0 font-black uppercase leading-none"><X size={20} /></button>
            </div>
            <div className="p-6 max-h-[60vh] overflow-y-auto no-scrollbar text-slate-800 text-left uppercase font-black">
               {shiftDefinitions.map(def => (
                <div key={def.id} onClick={() => togglePrintShift(def.id)} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border-2 transition-all mb-2 ${selectedPrintShifts.includes(def.id) ? 'border-blue-600 bg-blue-50' : 'border-slate-100 bg-white'}`}>
                  ...
                  <div className="text-xs font-black uppercase text-slate-800 text-left truncate leading-none">{String(def.label)}</div>
                </div>
               ))}
            </div>
            <div className="p-6 bg-slate-50 border-t uppercase font-black leading-none shrink-0"><button onClick={handlePrintAction} disabled={selectedPrintShifts.length === 0} className="w-full py-4 bg-blue-600 text-white font-black rounded-xl uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all leading-none">Proses</button></div>
          </div>
        </div>
      )}

      {selectedDate && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 bg-slate-900/50 backdrop-blur-md animate-in fade-in no-print text-slate-800 uppercase font-black leading-none">
          <div className="bg-white w-full max-w-xl rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 max-h-[90vh] flex flex-col text-slate-800 text-left uppercase font-black leading-none">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/80 gap-4 text-slate-800 shrink-0 text-left font-black uppercase">
              <div className="text-left text-slate-800 uppercase font-black leading-tight text-left uppercase font-black"><h3 className="text-lg leading-none text-left uppercase font-black">Atur Jadual</h3><p className={`font-bold text-[9px] ${holidays[selectedDate] ? 'text-red-500' : 'text-slate-500'} mt-1 tracking-widest text-left uppercase font-black`}>📅 {new Date(selectedDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
              <button onClick={() => setSelectedDate(null)} className="p-2 bg-white rounded-full shadow-sm border shrink-0 font-black uppercase leading-none"><X size={20} /></button>
            </div>
            <div className="p-4 md:p-6 overflow-y-auto no-scrollbar flex-1 text-slate-800 text-left uppercase font-black leading-none">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-800 text-left uppercase font-black leading-none">
                {shiftDefinitions.map(type => (
                  <div key={type.id} className="p-4 rounded-2xl border bg-slate-50 shadow-sm text-slate-800 text-left uppercase font-black leading-none">
                    <div className="flex items-center justify-between mb-3 pb-2 border-b text-slate-800 font-black uppercase leading-none"><div className="flex items-center gap-2 text-[10px] uppercase text-slate-800 font-black leading-none"><div className={`w-2.5 h-2.5 rounded-full ${getShiftColor(type.color, 'bg')}`}></div>{String(type.label)}</div><span className="text-[10px] bg-white px-2 py-0.5 rounded-full border text-slate-800 font-black leading-none">{(shifts[selectedDate]?.[type.id] || []).length}</span></div>
                    <div className="relative mb-3 uppercase font-black leading-none"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-300 font-black uppercase" size={12} /><input type="text" value={modalSearchTerms[type.id] || ""} onChange={(e) => setModalSearchTerms({...modalSearchTerms, [type.id]: e.target.value})} placeholder="Cari..." className="w-full pl-8 pr-3 py-2 bg-white border border-slate-200 rounded-lg font-bold text-[10px] outline-none text-slate-800 shadow-inner uppercase font-black leading-none" /></div>
                    <div className="space-y-1.5 text-left text-slate-800 uppercase font-black leading-none">
                      {(shifts[selectedDate]?.[type.id] || []).map(st => (<div key={st.staffId} onClick={() => toggleStaffOnShift(selectedDate, type.id, st.staffId)} className={`p-2 rounded-xl bg-white border shadow-sm flex items-center justify-between transition-all cursor-pointer hover:bg-red-50 hover:border-red-200 text-slate-800 text-left uppercase font-black leading-none`}><span className="text-[10px] font-black uppercase truncate text-slate-800 uppercase font-black leading-none">{String(st.staffName)}</span><CheckCircle2 size={14} className="text-emerald-500" /></div>))}
                      {staff.filter(s => String(s.name || '').toUpperCase().includes((modalSearchTerms[type.id] || '').toUpperCase()) && !(shifts[selectedDate]?.[type.id] || []).some(a => a.staffId === s.id)).slice(0, 5).map(s => (<div key={s.id} onClick={() => toggleStaffOnShift(selectedDate, type.id, s.id)} className="p-2 rounded-xl border border-dashed border-slate-200 flex items-center justify-between cursor-pointer opacity-60 hover:opacity-100 hover:bg-blue-50 text-slate-800 text-left uppercase font-black leading-none"><span className="text-[10px] uppercase font-bold text-slate-800 uppercase font-black leading-none">{String(s.name)}</span><Plus size={14} className="text-slate-300" /></div>))}
                    </div>
                  </div>
                ))}
              </div>
            </div>
            <div className="p-6 bg-slate-50 border-t shrink-0 uppercase font-black leading-none"><button onClick={() => setSelectedDate(null)} className="w-full py-4 bg-slate-900 text-white font-black rounded-xl uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all leading-none uppercase font-black">Simpan</button></div>
          </div>
        </div>
      )}
