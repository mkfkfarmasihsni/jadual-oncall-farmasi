...
import { 
  ...
  addDoc,
  updateDoc
} from 'firebase/firestore';import React, { useState, useEffect, useMemo } from 'react';
import { initializeApp } from 'firebase/app';
import { 
  getFirestore, 
  collection, 
...
const App = () => {
      ...
      <div className="min-h-screen bg-white p-8 font-sans text-black text-left">
        ...
        {printFormat === 'ot' && searchTerm.trim() && filteredStats ? (
          <div className="space-y-6">
            <div className="p-4 bg-slate-50 border border-slate-300 rounded-lg text-left uppercase">
              <h2 className="text-lg font-black underline mb-4 text-center text-black">Penyata Rekod Tugasan Individu</h2>
              <table className="w-full text-sm text-left text-black">
                <tbody>
                  <tr><td className="w-40 py-1 font-bold text-black uppercase leading-tight">Nama Kakitangan:</td><td className="py-1 uppercase font-black text-black">{String(filteredStats.name)}</td></tr>
                  <tr><td className="py-1 font-bold text-black uppercase leading-tight">Unit / Seksyen:</td><td className="py-1 uppercase text-left text-black">{String(filteredStats.unit || 'TIADA UNIT')}</td></tr>
                  <tr><td className="py-1 font-bold text-black uppercase leading-tight">Jumlah Hari:</td><td className="py-1 text-left text-black">{individualDutyList.length} Hari</td></tr>
                </tbody>
              </table>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 no-print flex flex-col">
        ...
        {activeTab === 'calendar' ? (
           <div className="space-y-4 md:space-y-6 animate-in fade-in">
             <div className="flex flex-col gap-4 text-slate-800 text-left">
                <div className="relative w-full max-w-md">
                  ...
                  <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Cari nama staf dalam jadual..." className="w-full pl-10 pr-10 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-xs md:text-sm outline-none shadow-sm focus:ring-2 focus:ring-blue-500 text-slate-700" />
              ...
              {/* INFO MASA SYIF (FUNGSI BARU: Klik syif dalam kalendar akan tunjuk info di sini) */}
              {activeShiftInfo && (
                          ...
                          {shiftDefinitions.map(type => {
                            ...
                            return (
                              <div 
                                ...
                                className={`px-1 md:px-2 py-0.5 md:py-1.5 rounded-md md:rounded-xl text-[7px] md:text-[10px] font-black truncate border shadow-sm flex items-center gap-1 ${getShiftColor(type.color, 'light')} border-current/10 hover:brightness-95 transition-all`}
                              >
          ...
          <div className="space-y-4 md:space-y-6 animate-in fade-in text-slate-800">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm gap-4 text-slate-800 text-left">
              <div className="text-left text-slate-800 uppercase">
                <h2 className="text-xl md:text-2xl font-black leading-tight uppercase">Pergerakan Staf</h2>
                <p className="text-slate-500 font-medium text-[10px] tracking-widest mt-1 uppercase">Rekod pergerakan harian kakitangan farmasi</p>
              </div>
            ...
            <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden text-slate-800">
              ...
              <div className="grid grid-cols-7 gap-px bg-slate-100 text-slate-800">
                {calendarDays.map((dateStr, idx) => {
                  if (!dateStr) return <div key={`empty-${idx}`} className="bg-slate-50/20 h-24 md:h-40 text-slate-800"></div>;
          ...
          <div className="space-y-6 animate-in fade-in text-slate-800 max-w-4xl mx-auto">
             <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 md:p-8 text-slate-800 text-left">
              <h2 className="text-xl md:text-2xl font-black mb-6 uppercase text-left">Maklumat Jadual</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-slate-800">
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-1 block text-left">Tajuk Utama</label><input type="text" value={appSettings.title} onChange={(e) => setAppSettings({...appSettings, title: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none text-slate-800 shadow-inner" /></div>
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-1 block text-left">Sub-Tajuk</label><input type="text" value={appSettings.subtitle} onChange={(e) => setAppSettings({...appSettings, subtitle: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none text-slate-800 shadow-inner" /></div>
                <div className="md:col-span-2 flex justify-end mt-2"><button onClick={() => updateAppSettings(appSettings.title, appSettings.subtitle)} className="px-10 py-3.5 bg-slate-900 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all text-xs uppercase">Simpan Maklumat</button></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 md:p-8 text-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-slate-800 text-left">
                {shiftDefinitions.map(def => (
                  <div key={def.id} className="p-6 rounded-[2rem] border border-slate-100 bg-slate-50 group transition-all hover:bg-white hover:shadow-xl text-left">
                    ...
                    <h3 className="font-black text-slate-800 text-xs md:text-sm uppercase leading-tight font-black">{String(def.label)}</h3><p className="text-[10px] font-bold text-slate-400 mt-1 uppercase whitespace-pre-line leading-tight text-left">{String(def.time || "Masa belum diset")}</p>
      ...
      {/* Modal Detail Pergerakan Harian (FUNGSI BARU) */}
      {viewingDayMovements && (
          ...
          <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden text-slate-800 max-h-[95vh] flex flex-col">
            ...
            <form onSubmit={saveMovement} className="p-6 space-y-5 overflow-y-auto no-scrollbar flex-1 text-slate-800">
              <div className="space-y-2 text-slate-800 text-left">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-1 block uppercase">Staf</label>
                {!newMovement.staffId ? (
                    ...
                    {movementModalSearch && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-2xl z-[140] overflow-hidden max-h-48 overflow-y-auto text-left">
                        {filteredMovementStaff.map(s => (
                          <div key={s.id} onClick={() => { setNewMovement({...newMovement, staffId: s.id, staffName: String(s.name)}); setMovementModalSearch(''); }} className="p-3.5 hover:bg-blue-50 cursor-pointer border-b border-slate-50 transition-colors flex items-center gap-3 text-left"><div className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-[10px] uppercase shrink-0">{String(s.name).substring(0, 2)}</div><span className="font-bold text-xs text-slate-700 uppercase leading-none truncate">{String(s.name)}</span></div>
                        ))}
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="p-3.5 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between text-slate-800 animate-in zoom-in-95 shadow-sm">
                    <div className="flex items-center gap-3 overflow-hidden"><div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-lg font-black text-xs shrink-0"><UserCheck size={16} /></div><span className="font-black text-blue-700 uppercase text-xs truncate leading-none">{String(newMovement.staffName)}</span></div>
              ...
              <div className="space-y-1.5 text-left text-slate-800">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-1 block">Jenis</label>
      ...
      {showPrintOptions && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden text-slate-800">
             <div className="p-6 border-b flex justify-between items-center bg-slate-50/80 text-slate-800">
              <div className="text-left uppercase font-black tracking-tight"><h3 className="text-lg leading-none">Cetak Laporan</h3></div>
            ...
            <div className="p-6 bg-slate-50 border-t"><button onClick={handlePrintAction} disabled={selectedPrintShifts.length === 0} className="w-full py-4 bg-blue-600 text-white font-black rounded-xl uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all">Proses Cetakan</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 bg-slate-900/50 backdrop-blur-md animate-in fade-in no-print text-slate-800">
          <div className="bg-white w-full max-w-xl rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 max-h-[90vh] flex flex-col text-slate-800">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/80 gap-4 text-slate-800 shrink-0">
              <div className="text-left text-slate-800 uppercase font-black leading-tight"><h3 className="text-lg leading-none">Atur Jadual</h3><p className={`font-bold text-[9px] ${holidays[selectedDate] ? 'text-red-500' : 'text-slate-500'} mt-1 tracking-widest`}>📅 {new Date(selectedDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
